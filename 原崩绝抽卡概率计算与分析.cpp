#include<stdio.h>
#include<stdlib.h>           //原神、崩坏星穹铁道、绝区零等游戏抽卡模型完全破解代码。
#include<math.h>
#define GENSHIN_IMPACT 0     //0代表原神，1代表星穹铁道，2代表绝区零，3代表鸣潮
...
int main() {
	...
	printf("输入数字选择游戏，0为原神，1为崩坏星穹铁道，2为绝区零，3为鸣潮\n");
	int flag;
    scanf("%d",&flag);
	int char_num; int gear_num;int num_char,num_gear;
	printf("输入抽取的角色数目和武器数目，以单空格隔开(注意角色满命或满魂是7个不是6个)\n");
	...
	if (judge1 && judge2 && judge3) {
		...
		printf("参考值:1sigma,0.6826;2sigma,0.9544;3sigma,0.9974\n");
	}
	free(char_up_pos);
	free(gear_up_pos);
	for (int i = 0; i < num_chars + 1; i++) {
